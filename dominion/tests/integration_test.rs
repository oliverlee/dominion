...
use dominion::{Arena, CardKind, KingdomSet, Turn};
...
fn play_all_treasures(arena: &mut Arena, _: usize) {
    for &card in &[CardKind::Gold, CardKind::Silver, CardKind::Copper] {
        loop {
            if arena.play_card(card).is_err() {
                break;
            }
        }
    }
...
fn big_money(arena: &mut Arena, player_id: usize) {
    ...
    println!(
        "p{} playing: {:?}",
        player_id,
        "" //FIXME arena.view(Location::Play { player_id }).unwrap()
    );
...
fn play_big_money() {
    ...
    while !arena.is_game_over() {
        ...
        println!(
            "p1 discard pile: {:?}",
            "" // FIXME arena.view(Location::Discard { player_id }).unwrap()
        );
        println!(
            "p1 hand: {:?}",
            "" // FIXME arena.view(Location::Hand { player_id }).unwrap()
        );
        println!();
