...
impl Supply {
    ...
    fn get_entry<'a>(&'a self, index: usize) -> Option<EntryRef<'a>> {
        ...
        entry.map(|(ref k, ref v)| (*k, v))
    }

    fn get_entry_mut<'a>(&'a mut self, index: usize) -> Option<EntryMut<'a>> {
        let n = self.kingdom_cards.len();
        let entry = if index < n {
    ...
    fn add_count(&mut self, index: usize, count: i32) -> Result<CardKind> {
        ...
        let sum = *supply_count as i32 + count;
        if sum >= 0 {
            *supply_count = sum as usize;
            Ok(card)
        } else {
...
impl Location for Supply {
    ...
    fn move_all(&mut self, other: &mut impl Location, indices: &[usize]) -> Result<()> {
        ...
        if runs.iter().all(|(&&index, &count)| {
            self.get_entry(index)
                .map(|(_, &supply_count)| supply_count >= count)
                .unwrap_or(false)
        }) {
            for (&index, count) in runs {
                self.add_count(index, -(count as i32)).unwrap();
